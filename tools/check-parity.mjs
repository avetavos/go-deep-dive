...
//   6. no Thai characters leak into a fenced ```go code block or a code
...
for (const enPath of files) {
  ...
  // 6. no Thai characters inside a fenced ```go block or code literal in TH file
  fencedGoBlocks(thSrc).forEach((block, i) => {
    if (THAI_RE.test(block)) {
      report(`${thPath}: Thai characters inside fenced \`\`\`go block #${i}`);
    }
  });
