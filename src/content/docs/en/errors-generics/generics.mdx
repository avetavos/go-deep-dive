...
Two rules to remember: **interface methods cannot declare type parameters**, and a generic method therefore cannot be used to satisfy an interface. Generic methods need Go 1.27+ (`go 1.27` in `go.mod`); the in-browser playground runs an older interpreter, so try these on go.dev/play.
