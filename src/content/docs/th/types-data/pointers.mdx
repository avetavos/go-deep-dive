...
- **pointer ใน Go อันตรายเหมือน C** — Go ไม่มี pointer arithmetic และ GC จัดการ lifetime ให้ — use-after-free เป็นไปไม่ได้ แต่ pointer ที่ยังชี้ไปยัง element เก่าหลัง `append` reallocate backing array ใหม่นั้นยังเกิดได้ (ดู [Slices](/go/th/types-data/slices/))
