...
สังเกตเวอร์ชัน: `go mod init` ของ Go 1.27 จงใจเขียน `go 1.26.0` ต่ำกว่า toolchain หนึ่ง minor release เพื่อให้ module ใหม่เอี่ยม build บน Go รุ่นก่อนได้ด้วย เมื่อต้องใช้ API ที่มีเฉพาะ 1.27 ให้ยกบรรทัดขึ้น:
...
สามบรรทัดที่ควรดูใกล้ ๆ:

- **`go 1.27`** คือเวอร์ชันของภาษาและ library ที่ module นี้เขียนขึ้นมาเทียบกับ — compiler บังคับใช้ และตั้งแต่ Go 1.27 การตรวจ `stdversion` ของ `go test` จะปฏิเสธการเรียก API ที่ใหม่กว่าบรรทัดนี้
